 ...
 *
 *  $Log: esdcan.c,v $
 *  Revision 1.167  2015/07/13 18:56:47  stefanm
 *  Removed the insidious support for the "baud" module parameter
 *  that is only present for the i82527 based boards (gitane, pc104_200i).
 *  The "/bin/sh" shell will suffice to execute the inodes script.
 *
...
static char* rcsid __attribute__((unused,used)) = "$Id: esdcan.c,v 1.167 2015/07/13 18:56:47 stefanm Exp $";
#elif ((__GNUC__ > 2) || (__GNUC__ == 2 && __GNUC_MINOR__ >=7))
static char* rcsid __attribute__((unused)) = "$Id: esdcan.c,v 1.167 2015/07/13 18:56:47 stefanm Exp $";
#else  /* No or old GNU compiler */
# define USE(var) static void use_##var(void *x) {if(x) use_##var((void *)var);}
static char* rcsid = "$Id: esdcan.c,v 1.167 2015/07/13 18:56:47 stefanm Exp $";
USE(rcsid);
